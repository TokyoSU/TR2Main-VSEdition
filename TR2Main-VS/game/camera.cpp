...
void MoveCamera(GAME_VECTOR* destination, int speed) {
...
void ClipCamera(int* x, int* y, int* z, int tx, int ty, int tz, int left, int top, int right, int bottom) {
...
void ShiftCamera(int* x, int* y, int* z, int tx, int ty, int tz, int left, int top, int right, int bottom) {
	...
	if (Camera.targetSquare < squareL + squareT) {
		*x = left;
		if (Camera.targetSquare >= squareL)
			*y = ty + (top >= bottom ? phd_sqrt(Camera.targetSquare - squareL) : -(long)phd_sqrt(Camera.targetSquare - squareL));
	}
	else {
				...
				*x = left;
				...
				if (2 * Camera.targetSquare < squareR) {
					if (2 * Camera.targetSquare >= squareT) {
						*x = tx + (left < right ? phd_sqrt(2 * Camera.targetSquare - squareT) : -(long)phd_sqrt(2 * Camera.targetSquare - squareT));
						*y = top;
					}
				}
				else {
...
void SmartShift(GAME_VECTOR* goal, CB_SMARTCAM shift) {
	ROOM_INFO* room;
	BOX_INFO* box;
...
int ShiftClamp(GAME_VECTOR* pos, int clamp) {
	FLOOR_INFO* floor;
	BOX_INFO* box;
