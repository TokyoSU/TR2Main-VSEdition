...
bool OpenGameRegistryKey(LPCTSTR key) {
	if( key == NULL )
...
LONG CloseGameRegistryKey() {
  return CloseRegistryKey();
}

bool SE_WriteAppSettings(APP_SETTINGS *settings) {
	LPCTSTR guidString;
...
int SE_ReadAppSettings(APP_SETTINGS *settings) {
	bool rc;
	GUID guid;
...
void SE_GraphicsTestFinish() {
	RenderFinish(true);
	WinVidFinish();
	WinVidHideGameWindow();
}

int SE_GraphicsTestExecute() {
	return 0;
}
...
void SE_DefaultGraphicsSettings() {
...
void SE_SoundTestFinish() {
	WinSndFinish();
}

int SE_SoundTestExecute() {
...
bool SE_ShowSetupDialog(HWND hParent, bool isDefault) {
...
void SE_GraphicsDlgFullScreenModesUpdate(HWND hwndDlg) {
	HWND comboBox;
...
void SE_GraphicsAdapterSet(HWND hwndDlg, DISPLAY_ADAPTER_NODE *adapter) {
	ChangedAppSettings.PreferredDisplayAdapter = adapter;
	SE_GraphicsDlgFullScreenModesUpdate(hwndDlg);
}

void SE_GraphicsDlgUpdate(HWND hwndDlg) {
	HWND hItem;
...
void SE_GraphicsDlgInit(HWND hwndDlg) {
...
void SE_SoundAdapterSet(HWND hwndDlg, SOUND_ADAPTER_NODE *adapter) {
	ChangedAppSettings.PreferredSoundAdapter = adapter;
}

void SE_SoundDlgUpdate(HWND hwndDlg) {
	HWND hItem;
...
void SE_SoundDlgInit(HWND hwndDlg) {
	HWND comboBox;
	DWORD addedIndex, selectedIndex;
...
void SE_ControlsJoystickSet(HWND hwndDlg, JOYSTICK_NODE *joystick) {
	ChangedAppSettings.PreferredJoystick = joystick;
}

void SE_ControlsDlgUpdate(HWND hwndDlg) {
...
void SE_ControlsDlgInit(HWND hwndDlg) {
	HWND comboBox;
	DWORD addedIndex, selectedIndex;
...
void SE_OptionsDlgUpdate(HWND hwndDlg) {
	LPCTSTR lpString;
...
void SE_OptionsStrCat(LPTSTR *dstString, bool isEnabled, bool *isNext, LPCTSTR srcString) {
	if( !isEnabled )
		return;
...
void SE_AdvancedDlgUpdate(HWND hwndDlg) {
...
void SE_AdvancedDlgInit(HWND hwndDlg) {
	SE_AdvancedDlgUpdate(hwndDlg);
}

HWND SE_FindSetupDialog() {
