...
void SE_ReleaseBitmapResource(BITMAP_RESOURCE* bmpRsrc) {
...
void SE_LoadBitmapResource(BITMAP_RESOURCE* bmpRsrc, LPCTSTR lpName) {
...
void SE_DrawBitmap(BITMAP_RESOURCE* bmpRsrc, HDC hdc, int x, int y) {
	int width, height;
	HPALETTE hPalette;
...
void SE_UpdateBitmapPalette(BITMAP_RESOURCE* bmpRsrc, HWND hWnd, HWND hSender) {
	if (bmpRsrc->hPalette != NULL && hSender != hWnd)
		SE_ChangeBitmapPalette(bmpRsrc, hWnd);
}

void SE_ChangeBitmapPalette(BITMAP_RESOURCE* bmpRsrc, HWND hWnd) {
	HDC hdc;
	HPALETTE hPalette;
...
bool SE_RegisterSetupWindowClass() {
	WNDCLASSA wndClass;
...
void SE_PassMessageToImage(HWND hWnd, UINT uMsg, WPARAM wParam) {
